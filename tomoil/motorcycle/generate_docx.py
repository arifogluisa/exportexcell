...
def extract_data_from_excell(file_name: str, sheet_name: str) -> None:
    ...
    for main_row_index in range(3, len(df), 9):
        ...
        jaso = tuple(main_row.items())[9][1]
        oem_claims = tuple(main_row.items())[10][1]

        performance_claims = get_performance_claims(api, jaso, oem_claims)
        row_item = {
            "pds_no": pds_no,
...
def get_performance_claims(
    ...
    if jaso not in ["Not available", "Not applicable", "", None]:
        result.append(f"JASO {jaso}")
...
def generate_docx(
    ...
    save_folder = r"../docs/Motorcycle oil"
    product_folder = (
        f'{product_name.replace("/", "-")}_{sae_grade}'
        if sae_grade not in ["Not available", "Not applicable", "", None]
        else product_name.replace("/", "-")
    )
    for i in range(1, 4):
...
excell_files = extract_data_from_excell("../exc_temp.xlsx", "Motorcyle oil")
