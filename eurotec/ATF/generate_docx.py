...
def extract_data_from_excell(file_name: str, sheet_name: str) -> None:
    ...
    for main_row_index in range(3, len(df), 9):
        ...
        recommendations = tuple(main_row.items())[7][1]

        performance_claims = get_performance_claims(oem_claims)
        recommends = get_recommends(recommendations)
        row_item = {
            'pds_no': pds_no,
...
def get_recommends(recommendations: str) -> list:
    if recommendations not in ['Not available', 'Not applicable', '', None]:
        recommendations = str(recommendations)
        recommendations = (
            recommendations.split(',')
            if ',' in recommendations
            else recommendations.split('\n')
        )
        return [item.strip() for item in recommendations]
    return []
...
def generate_docx(
    ...
    recommends_place = "Alco-recommends"
    recommends_title = "ALCO LLC recommends this product for vehicles with the following specifications:"
    ...
    for paragraph in doc.paragraphs:
        ...
        if recommends_place in paragraph.text:
            paragraph.clear()
            if recommends:
                paragraph.text = "\n".join([f"• {item}" for item in recommends])
                for run in paragraph.runs:
                    run.font.name = "Arial"
            else:
                for paragraph_inner in doc.paragraphs:
                    if recommends_title in paragraph_inner.text:
                        paragraph_inner.clear()
    ...
    save_folder = r"../docs/ATF"
    product_folder = product_name.replace("/", "-").replace('\n', '')
    for i in range(1, 4):
...
excell_files = extract_data_from_excell('../exc_temp.xlsx', 'ATF')
